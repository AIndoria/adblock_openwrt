...
# Indoria
# ADBLOCKER FOR OPENWRT, Original script by tablespoon, now with a consolidated hostfile which auto updates to remove redundancies
# Autmatically downloads the hostlist every few days and blocks the data being sent to "Problematic" servers.
...
if ! grep -Fq "$SCRIPT_NAME" /etc/crontabs/root 2>/dev/null; then
	...
	cat >>/etc/crontabs/root <<-:EOF:
		# Downloads the new hostlist every TUE at 3:$(printf "%02d" "$DELAY") AM
